...
export function SortableTable<T>({ rows, columns, initialSort, initialDir = 'desc', rowKey, onRowClick }: Props<T>) {
            ...
            {columns.map((col, i) => {
              ...
              return (
                <th
                  ...
                  className={`px-3 py-2.5 font-semibold whitespace-nowrap text-ink-2 ${alignClass[col.align ?? (i === 0 ? 'left' : 'right')]} ${
                    col.sortValue ? 'cursor-pointer select-none hover:text-ink' : ''
                  } ${isSticky ? 'sticky left-0 z-10 bg-surface-2' : ''}`}
                >
            ...
            >
              {columns.map((col, i) => {
                ...
                return (
                  <td
                    key={col.key}
                    className={`px-3 py-2.5 ${alignClass[col.align ?? (i === 0 ? 'left' : 'right')]} ${
