...
// Static pitch chrome (markings + goal frame), ported from shotzones pitchMarkings/goalFrame.
...
interface Tip { key: string; x: number; y: number }
...
export function TeamShotMap({ team }: { team: string }) {
  ...
  const analysis: Analysis = useMemo(
    () => analyse(venueFilterShots(windowShots(teamShots, win), venue), mode),
    [teamShots, win, venue, mode],
  )

  const selectMode = (m: ShotMode) => {
    if (m === 'for') setWantFor(true)
    setMode(m)
    setTip(null)
  }
  ...
  const scatter = venueFilterShots(windowShots(teamShots, win), venue).filter((s) => s.situation !== 'Penalty')

  if (active.loading && !active.data) {
    return <div className="py-8 text-sm text-ink-2">Loading shot data…</div>
  }
  if (!concededQ.loading && teamShots.length === 0 && mode === 'against') {
    return (
      <div className="py-8 text-sm text-ink-2">
        No shot-level data yet for {teamFullNames[team] || team}.
      </div>
    )
  }
  ...
  return (
    <div>
      ...
      <div className="flex items-stretch gap-2">
        ...
        <div ref={pitchRef} className="relative mx-auto w-full max-w-[520px]" onPointerLeave={() => setTip(null)}>
          ...
          {Object.entries(ZONE_SHAPES).map(([key, s]) => {
            ...
            return (
              <div key={key} className="shotmap-zone-label" style={{ left: `${left}%`, top: `${top}%` }}>
                {pct}%
              </div>
            )
          })}
          ...
          {tip && (() => {
            ...
            return (
              <div
                className="pointer-events-none absolute z-20 w-[200px] rounded-md border border-line-mid bg-surface-3 px-3 py-2 shadow-float"
                style={{ left: tip.x, top: tip.y }}
              >
                <div className="mb-1 text-xs font-semibold text-ink">{title}</div>
                {lines.map((l, i) => (
                  <div key={i} className="text-[11px] text-ink-2">{l}</div>
                ))}
              </div>
            )
          })()}
      ...
      <p className="mt-3 text-xs text-ink-3">% = share of {mm.noun} by zone · shading follows the same share</p>
    </div>
  )
}
