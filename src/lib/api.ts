...
// yet (e.g. pre-season).
async function getCurrentGwFallback(): Promise<number> {
  if (!bootstrapCache) {
  ...
  const finished = events.filter((e: any) => e.finished);
  if (finished.length) return Math.max(...finished.map((e: any) => e.id));
  return events.length ? events[0].id : 1;
}

// Full entry payload: manager name, current GW, classic league memberships
async function fetchEntry(teamId: string | number): Promise<any> {
  const entryRes = await fplFetch(`https://fantasy.premierleague.com/api/entry/${teamId}/`);
  return entryRes.json();
}

async function getGwForTeam(teamId: string | number): Promise<number> {
...
async function fetchLeagueStandings(leagueId: string | number): Promise<any> {
...
async function fetchPicksCached(teamId: string | number, gw: string | number): Promise<any> {
...
async function fetchEntryHistory(teamId: string | number): Promise<any> {
  try {
