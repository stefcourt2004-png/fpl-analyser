// narrative.js — turns raw tables into sentences: player verdicts for the
// player page hero, and league-level stories for the Home briefing.
// Pure functions, no DOM and no data.js import (same contract as engine.js).
import { starsToNum } from '../util.js';
import { blendedPpg, blendedScore } from './rules.js';
import { makeTeamEase } from './engine.js';
...
function buildPlayerBundle(ref, data) {
  const byRef = (rows) => (rows || []).find(
    typeof ref === 'number' ? (x) => x.element === ref : (x) => x.web_name === ref);
  const r = byRef(data.ratings);
  if (!r) return null;
  return {
    r,
    m: (data.metrics || []).find(x => x.element === r.element) || null,
    std: (data.seasonToDate || []).find(x => x.element === r.element) || null,
    p4: (data.personas4 || []).find(x => x.element === r.element) || null,
    priceRisk: (data.priceRisk || []).find(x => x.element === r.element) || null,
...
function ratingPercentile(bundle, data) {
  const { r, hasFixtures } = bundle;
  const broad = (p) => (p === 'GKP' ? 'GKP' : p === 'DEF' ? 'DEF' : 'ATT');
  const peers = (data.ratings || []).filter(x =>
    x.season_ok && broad(x.position) === broad(r.position) &&
    ((num(x.gw4_start_rate) ?? num(x.season_start_rate) ?? 0) >= 0.7));
  const scoreOf = hasFixtures
    ? (x) => (blendedScore(x) ?? 0) * (num(x.next4_fixture_factor) ?? 1)
    : (x) => num(x.season_overall_score) ?? 0;
  const mine = scoreOf(r);
  if (!peers.length || mine == null) return null;
  const below = peers.filter(x => scoreOf(x) < mine).length;
  return Math.min(99, Math.round(below / Math.max(peers.length - 1, 1) * 100));
}

function verdictLabel(score, hasFixtures, hasRisk) {
  const labels = hasFixtures
...
function buildPlayerVerdict(bundle, data) {
  ...
  if (hasFixtures) {
    const next4 = bundle.fixtureEase
      .filter(f => f.team === r.team)
      .sort((a, b) => a.gw - b.gw)
      .slice(0, 4);
    if (next4.length >= 2) {
      const kind = next4.filter(f => num(f.fdr) != null && f.fdr <= 2);
      const avgFdr = next4.reduce((s, f) => s + (num(f.fdr) ?? 3), 0) / next4.length;
      const list = (fs) => fs.map(f => `${f.opponent} (${f.venue})`).join(', ');
      if (kind.length >= 2) {
        bullets.push({
...
function buildLeagueStories(data) {
...
function playerStory(title, iconId, tone, row, data, extraBullets = []) {
...
function weeklyStories(data) {
  const stories = [];

  // 1 — the captaincy verdict: best form × fixtures over the next 4
  const captains = (data.ratings || [])
    .filter(p => p.season_ok && num(p.next4_score) != null)
    .sort((a, b) => (b.next4_score || 0) - (a.next4_score || 0));
  if (captains.length) {
  ...
  const teams = [...new Set((data.fixtureEase || []).map(f => f.team))];
  let best = null;
  for (const team of teams) {
    const near = teamEase(team, 0, 3), later = teamEase(team, 3, 3);
  ...
  const hot = (data.seasonToDate || [])
    .filter(p => p.streak === '🔥 Hot' && num(p.xgi_delta) != null && p.xgi_delta > 0)
...
function recapStories(data) {
  const rated = (data.ratings || []).filter(p => p.season_ok);
  const stories = [];

  const top = [...rated].sort((a, b) => (b.season_overall_score || 0) - (a.season_overall_score || 0))[0];
  if (top) {
  ...
  const value = [...rated]
    .filter(p => num(p.season_value_score) != null && num(p.price) != null)
    .sort((a, b) => (b.season_value_score || 0) - (a.season_value_score || 0))[0];
  ...
  const clinical = [...rated]
    .filter(p => ['MID', 'FWD'].includes(p.position) && num(p.season_finishing_skill_score) != null)
    .sort((a, b) => (b.season_finishing_skill_score || 0) - (a.season_finishing_skill_score || 0))[0];
